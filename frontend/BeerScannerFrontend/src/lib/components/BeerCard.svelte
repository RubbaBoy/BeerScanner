<script lang="ts">
  ...
  let {beer, showActions = true, date = undefined, dateCurrent = true} = $props<{
    beer: Beer;
    showActions?: boolean;
    date?: string;
    dateCurrent?: boolean;
  ...
  let dateFormatted = formatFromNowDate(date);
...
<div class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300 flex flex-col">
    ...
    <div class="px-6 pb-4">
        ...
        {#if date}
            {#if dateCurrent}
                <p class="text-sm text-gray-500 mt-4">Added {dateFormatted}</p>
            {:else}
                <p class="text-sm text-gray-500 mt-4">Removed {dateFormatted}</p>
            {/if}
        {/if}
    </div>
</div>
